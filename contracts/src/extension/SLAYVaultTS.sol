...
import {SLAYERC20Checkpointed} from "./SLAYERC20Checkpointed.sol";
import {SLAYERC20Derivative} from "./SLAYERC20Derivative.sol";
import {SLAYERC4626} from "./SLAYERC4626.sol";
import {SLAYRegistryV2} from "../SLAYRegistryV2.sol";
import {SLAYRouterV2} from "../SLAYRouterV2.sol";
import {SLAYSlashToken} from "./SLAYSlashToken.sol";

contract SLAYVaultTS is Initializable, SLAYERC4626, ERC165Upgradeable, ISLAYVaultV2 {
    using SafeERC20 for IERC20;

    event SlashTokenCreated(address indexed slashToken, uint32 createdAtBlock);
    ...
    function decimals() public view override(IERC20Metadata, SLAYERC4626) returns (uint8) {
        return SLAYERC4626.decimals();
    }
    ...
    /// @inheritdoc ISLAYVaultV2
    function totalActiveStaked() external view override returns (uint256) {
    ...
    /// @inheritdoc IERC4626
    function maxWithdraw(address controller) public view virtual override(IERC4626, SLAYERC4626) returns (uint256) {
        RedeemRequestStruct memory request = _pendingRedemption[controller];
        if (request.claimableAt <= block.timestamp) {
    ...
    /// @inheritdoc IERC4626
    function maxRedeem(address controller) public view virtual override(IERC4626, SLAYERC4626) returns (uint256) {
        RedeemRequestStruct memory request = _pendingRedemption[controller];
        if (request.claimableAt <= block.timestamp) {
            return request.shares;
        }
     ...
     */
    function previewWithdraw(uint256) public pure virtual override(IERC4626, SLAYERC4626) returns (uint256) {
     ...
     * See https://eips.ethereum.org/EIPS/eip-7540#reversion-of-preview-functions-in-async-request-flows
     */
    function previewRedeem(uint256) public pure virtual override(IERC4626, SLAYERC4626) returns (uint256) {
        revert PreviewNotSupported();
    }
    ...
    function lockSlashing(uint256 amount) external override whenNotPaused {
        ...
        // create slash tokens
        slashTokens.push(address(_createDerivative(_name, _symbol)));
    ...
    /**
     * @dev Creates a new slash token contract linked to this token at current block height
     */
    function _createDerivative(string memory _name, string memory _symbol)
        internal
        override
        returns (SLAYERC20Derivative)
    {
        string memory derivativeName = string(abi.encodePacked("Slash ", _name));
        string memory derivativeSymbol = string(abi.encodePacked("s", _symbol));

        SLAYSlashToken derivative =
            new SLAYSlashToken(derivativeName, derivativeSymbol, uint32(block.number), address(this));

        emit SlashTokenCreated(address(derivative), uint32(block.number));
