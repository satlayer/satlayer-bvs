...
import {SLAYERC20Derivative, SLAYERC20DerivativeFactory} from "./SLAYERC20Derivative.sol";

/**
 * @dev adapted from OpenZeppelin's ERC20 implementation with checkpointing added to balances and totalSupply.
 */
abstract contract SLAYERC20Checkpointed is
    Initializable,
    Context,
    IERC20,
    IERC20Metadata,
    IERC20Errors,
    SLAYERC20DerivativeFactory
{
    ...
    function totalSupplyAt(uint32 blockNumber) public view virtual returns (uint256) {
        return _totalSupply.upperLookupRecent(blockNumber);
    }
    ...
    function balanceOfAt(address account, uint32 blockNumber) public view virtual returns (uint256) {
        return _balances[account].upperLookupRecent(blockNumber);
    }
