...
interface ISLAYRouterSlashingV2 {
    ...
    /// @dev Error thrown when guardrail attempts to approved/rejected the slashing request more than once.
    error GuardrailAlreadyApproved();

    /// @dev Error thrown when the guardrail has not approved the slashing request to be finalized.
    ...
    /// @dev Emitted when a slash request has been approved or rejected by the guardrail.
     ...
     * @param slashId The unique identifier for the slashing request.
     * @return Request The current active slashing request information.
     */
    ...
    /**
     * @notice For the guardrail to approve on a slashing request before slashing can be finalized.
    ...
    /**
     * @dev Cancel the slashing request by the same service that requested it to an operator.
