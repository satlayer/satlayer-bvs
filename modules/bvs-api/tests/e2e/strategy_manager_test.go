...
func (suite *strategyManagerTestSuite) test_QueryStrategyManager() {
	...
	resp, err := strategyManager.StakerDepositList(stakerAddr)
	assert.NoError(t, err, "StakerDepositList")
	assert.NotNil(t, resp, "response nil")
	t.Logf("StakerDepositList resp:%+v", resp)

	resp, err = strategyManager.StakerStrategyShares(stakerAddr, strategyAddr)
	assert.NoError(t, err, "StakerStrategyShares")
	assert.NotNil(t, resp, "response nil")
	t.Logf("StakerStrategyShares resp:%+v", resp)

	resp, err = strategyManager.StakerStrategyList(strategyAddr)
	assert.NoError(t, err, "StakerStrategyList")
	assert.NotNil(t, resp, "response nil")
	t.Logf("StakerStrategyList resp:%+v", resp)
