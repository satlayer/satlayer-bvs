...
type NodeApi struct {
	...
	ipPortAddr    string
	logger        elklogger.Logger
}

func NewNodeApi(bvsNodeName, bvsNodeSemVer, IpPortAddr string, logger elklogger.Logger) *NodeApi {
	nodeApi := &NodeApi{
...
func (api *NodeApi) Start() <-chan error {
	api.logger.Info("Starting node api server at address", elklogger.WithField("addr", api.ipPortAddr))
...
func (api *NodeApi) nodeHandler(w http.ResponseWriter, r *http.Request) {
	response := map[string]string{
		"node_name":    api.bvsNodeName,
		"spec_version": specSemVer,
		"node_version": api.bvsNodeSemVer,
	}
	err := jsonResponse(w, response)
	if err != nil {
		api.logger.Error("error in nodeVersionHandler", elklogger.WithField("err", err))
	}
}
...
func (api *NodeApi) healthHandler(w http.ResponseWriter, r *http.Request) {
	...
	default:
		...
		w.WriteHeader(http.StatusServiceUnavailable)
		api.logger.Error("unknown health status", elklogger.WithField("health", api.health))
	}
}
...
func (api *NodeApi) servicesHandler(w http.ResponseWriter, r *http.Request) {
	...
	err := jsonResponse(w, response)
	if err != nil {
		api.logger.Error("error in servicesHandler", elklogger.WithField("err", err))
	}
}

func (api *NodeApi) serviceHealthHandler(w http.ResponseWriter, r *http.Request) {
	suffix, found := strings.CutPrefix(r.URL.Path, baseUrl+"/node/services/")
	if !found {
	...
	for _, service := range api.nodeServices {
		if service.Id == serviceId {
			...
			default:
				w.WriteHeader(http.StatusServiceUnavailable)
				api.logger.Error("unknown service status", elklogger.WithField("serviceStatus", service.Status))
			}
			return
		}
	}
...
func run(logger elklogger.Logger, httpServer *http.Server) <-chan error {
	...
	go func() {
		logger.Info("node api server running", elklogger.WithField("addr", httpServer.Addr))
