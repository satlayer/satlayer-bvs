...
var testNodeApi = NewNodeApi("testBVS", "v0.0.1", "localhost:8080", logger)
...
func TestNodeHandler(t *testing.T) {
	req := httptest.NewRequest(http.MethodGet, "/satlayer/node", nil)
	...
	assert.Equal(
		t,
		"{\"node_name\":\"testBVS\",\"node_version\":\"v0.0.1\",\"spec_version\":\"v0.0.1\"}\n",
		string(data),
	)
}
...
func TestHealthHandler(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"partially healthy": {
			nodeApi: func() *NodeApi {
				nodeApi := NewNodeApi("testBVS", "v0.0.1", "localhost:8080", logger)
	...
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
			req := httptest.NewRequest(http.MethodGet, "/satlayer/node/health", nil)
			w := httptest.NewRecorder()
...
func TestServicesHandler(t *testing.T) {
	...
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
			req := httptest.NewRequest(http.MethodGet, "/satlayer/node/services", nil)
			w := httptest.NewRecorder()
...
func TestServiceHealthHandler(t *testing.T) {
	req := httptest.NewRequest(http.MethodGet, "/satlayer/node/services/123/health", nil)
	w := httptest.NewRecorder()
