...
import (
	...
	"github.com/satlayer/satlayer-bvs/bvs-api/tests/e2e"
...
func (s *DirectoryTestSuite) SetupSuite() {
	...
	delegationApi := NewDelegationImpl(chainIO, delegationManager.Address)
	s.Require().NoError(err, "setup keyring")
	accountPubKey := e2e.GetPubKeyFromKeychainByUid(chainIO, "operator1")
...
func (s *DirectoryTestSuite) test_RegisterBVS() {
	...
	bvsApi := NewBvsDirectory(chainIO, s.contrAddr)
...
func (s *DirectoryTestSuite) Test_UpdateMetadataURI() {
	...
	chainIO, err := s.chainIO.SetupKeyring(keyName, "test")
	assert.NoError(s.T(), err)
	txResp, err := NewBvsDirectory(chainIO, s.contrAddr).UpdateMetadataURI(context.Background(), "example.com")
...
func (s *DirectoryTestSuite) Test_CancelSalt() {
	...
	assert.NoError(s.T(), err)
	salt := "salt" + randomStr
	txResp, err := NewBvsDirectory(chainIO, s.contrAddr).CancelSalt(context.Background(), salt)
...
func (s *DirectoryTestSuite) Test_TransferOwnership() {
	...
	updateResp, err := NewBvsDirectory(chainIO, s.contrAddr).TransferOwnership(context.Background(), "bbn1yh5vdtu8n55f2e4fjea8gh0dw9gkzv7uxt8jrv")
	...
	recoverResp, err := NewBvsDirectory(recoverClient, s.contrAddr).TransferOwnership(context.Background(), "bbn1dcpzdejnywqc4x8j5tyafv7y4pdmj7p9fmredf")
	assert.NoError(t, err, "TestTransferOwnership")
...
func (s *DirectoryTestSuite) Test_Pause() {
	...
	bvsDirectory := NewBvsDirectory(chainIO, s.contrAddr)
	{
...
func (s *DirectoryTestSuite) Test_SetPauser() {
	...
	chainIO, err := s.chainIO.SetupKeyring(keyName, "test")
	assert.NoError(t, err)
	txResp, err := NewBvsDirectory(chainIO, s.contrAddr).SetPauser(context.Background(), "bbn1dcpzdejnywqc4x8j5tyafv7y4pdmj7p9fmredf")
	assert.NoError(t, err)
	assert.NotNil(t, txResp, "response nil")
	t.Logf("txResp:%+v", txResp)
}
...
func (s *DirectoryTestSuite) Test_SetUnpauser() {
	...
	chainIO, err := s.chainIO.SetupKeyring(keyName, "test")
	assert.NoError(t, err)
	txResp, err := NewBvsDirectory(chainIO, s.contrAddr).SetUnpauser(context.Background(), "bbn1dcpzdejnywqc4x8j5tyafv7y4pdmj7p9fmredf")
	assert.NoError(t, err)
	assert.NotNil(t, txResp, "response nil")
	t.Logf("txResp:%+v", txResp)
}
...
func (s *DirectoryTestSuite) Test_SetDelegationManager() {
	...
	chainIO, err := s.chainIO.SetupKeyring(keyName, "test")
	assert.NoError(t, err)
	txResp, err := NewBvsDirectory(chainIO, s.contrAddr).SetDelegationManager(context.Background(), s.delegationContrAddr)
	assert.NoError(t, err)
	assert.NotNil(t, txResp, "response nil")
	t.Logf("txResp:%+v", txResp)
}
...
func (s *DirectoryTestSuite) Test_QueryOperator() {
	...
	chainIO, err := s.chainIO.SetupKeyring(keyName, "test")
	assert.NoError(t, err)
	txResp, err := NewBvsDirectory(chainIO, s.contrAddr).QueryOperator("bbn1rt6v30zxvhtwet040xpdnhz4pqt8p2za7y430x", "bbn1rt6v30zxvhtwet040xpdnhz4pqt8p2za7y430x")
...
func (s *DirectoryTestSuite) Test_CalculateDigestHash() {
	...
	msgHashResp, err := NewBvsDirectory(chainIO, s.contrAddr).CalculateDigestHash(
		pubKey,
...
func (s *DirectoryTestSuite) Test_IsSaltSpent() {
	...
	chainIO, err := s.chainIO.SetupKeyring(keyName, "test")
	assert.NoError(t, err)
	txResp, err := NewBvsDirectory(chainIO, s.contrAddr).IsSaltSpent("bbn1rt6v30zxvhtwet040xpdnhz4pqt8p2za7y430x", "c2FsdDEyMzE4NTgyNzI1NDIyMDc5NDI4")
...
func (s *DirectoryTestSuite) Test_GetDelegationManager() {
	...
	chainIO, err := s.chainIO.SetupKeyring(keyName, "test")
	assert.NoError(t, err)
	txResp, err := NewBvsDirectory(chainIO, s.contrAddr).GetDelegationManager()
...
func (s *DirectoryTestSuite) Test_GetOwner() {
	...
	chainIO, err := s.chainIO.SetupKeyring(keyName, "test")
	assert.NoError(t, err)
	txResp, err := NewBvsDirectory(chainIO, s.contrAddr).GetOwner()
...
func (s *DirectoryTestSuite) Test_GetOperatorBVSRegistrationTypeHash() {
	...
	chainIO, err := s.chainIO.SetupKeyring(keyName, "test")
	assert.NoError(t, err)
	txResp, err := NewBvsDirectory(chainIO, s.contrAddr).GetOperatorBVSRegistrationTypeHash()
...
func (s *DirectoryTestSuite) Test_GetDomainTypeHash() {
	...
	chainIO, err := s.chainIO.SetupKeyring(keyName, "test")
	assert.NoError(t, err)
	txResp, err := NewBvsDirectory(chainIO, s.contrAddr).GetDomainTypeHash()
...
func (s *DirectoryTestSuite) Test_GetDomainName() {
	...
	chainIO, err := s.chainIO.SetupKeyring(keyName, "test")
	assert.NoError(t, err)
	txResp, err := NewBvsDirectory(chainIO, s.contrAddr).GetDomainName()
...
func (s *DirectoryTestSuite) test_GetBVSInfo() {
	...
	chainIO, err := s.chainIO.SetupKeyring(keyName, "test")
	assert.NoError(t, err)
	txResp, err := NewBvsDirectory(chainIO, s.contrAddr).GetBVSInfo("af0a809a3b8de8656c9b1af0389174a0ee49bf7094b84102171d4fe9f1d24770")
	assert.NoError(t, err, "TestGetBVSInfo")
	assert.NotNil(t, txResp, "response nil")
	t.Logf("txResp:%+v", txResp)

	// get non exist bvsHash will nil
	txResp, err = NewBvsDirectory(chainIO, s.contrAddr).GetBVSInfo("d2f665ee5cbc6e4d2fa992defb065fb1d51539db35654ed28feae24fcfa7cdbf")
