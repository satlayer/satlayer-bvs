...
type Monitor struct {
	...
	bvsDirectoryApi api.ETHBVSDirectory
	bvsSquaringApi  bvssquaringapi.BVSSquaring
...
func NewMonitor() *Monitor {
	...
	directoryContractABI, err := chainioabi.GetContractABI("../data/abi", "BVSDirectory")
	directoryContract := common.HexToAddress(core.C.Chain.BVSDirectory)
	ctx := context.Background()
	directoryApi := api.NewETHBVSDirectoryImpl(ethChainIO, directoryContract, directoryContractABI)
	txResp, err := directoryApi.GetBVSInfo(ctx, core.C.Chain.BVSHash)
	if err != nil {
		panic(err)
	}
	fmt.Printf("bvs contract: %s\n", txResp.BVSContract.String())
	bvsContractABI, err := chainioabi.GetContractABI("../data/abi", "BVSSquaring")
	if err != nil {
		panic(err)
	}
	bvsSquaringApi := bvssquaringapi.NewBVSSquaringImpl(ethChainIO, txResp.BVSContract, bvsContractABI)
	...
	return &Monitor{
		...
		bvsDirectoryApi: directoryApi,
		bvsSquaringApi:  bvsSquaringApi,
		chainIO:         ethChainIO,
		wallet:          wallet,
	}
}
...
func (m *Monitor) sendTaskResult(taskId uint64, result int64, operators string) error {
	...
	_, err := m.bvsSquaringApi.RespondToTask(context.Background(), m.wallet, taskId, result, operators)
	if err != nil {
		return err
	}
