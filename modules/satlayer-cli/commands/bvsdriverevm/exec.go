...
func RegBVS(userAddr, password, bvsContract string) {
	...
	txResp, err := s.BVSDriver.AddRegisteredBvsContract(ctx, wallet, bvsContractAddr)
	if err != nil {
		panic(err)
	}
...
func AcceptOwner(userAddr, password string) {
	...
	txResp, err := s.BVSDriver.AcceptOwnership(ctx, wallet)
	if err != nil {
		panic(err)
	}
...
func RenounceOwner(userAddr, password string) {
	...
	txResp, err := s.BVSDriver.RenounceOwnership(ctx, wallet)
	if err != nil {
		panic(err)
	}
...
func TransferOwner(userAddr, password, newOwner string) {
	...
	txResp, err := s.BVSDriver.TransferOwnership(ctx, wallet, newOwnerAddr)
	if err != nil {
		panic(err)
	}
