...
func RegBVS(userAddr, password, bvsHash, bvsContract string) {
	...
	txResp, err := s.Directory.RegisterBVS(ctx, wallet, bvsHash, bvsContractAddr)
	if err != nil {
		panic(err)
	}
...
func AcceptOwner(userAddr, password string) {
	...
	txResp, err := s.Directory.AcceptOwnership(ctx, wallet)
	if err != nil {
		panic(err)
	}
...
func RenounceOwner(userAddr, password string) {
	...
	txResp, err := s.Directory.RenounceOwnership(ctx, wallet)
	if err != nil {
		panic(err)
	}
...
func TransferOwner(userAddr, password, newOwner string) {
	...
	txResp, err := s.Directory.TransferOwnership(ctx, wallet, newOwnerAddr)
	if err != nil {
		panic(err)
	}
