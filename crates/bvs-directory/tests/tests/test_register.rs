...
//     pub(crate) mod setup;
// }
use super::helpers::{generate_babylon_public_key_from_private_key, mock_app};
use base64::{engine::general_purpose, Engine as _};
use bvs_testing::integration::*;
use cosmwasm_std::testing::{MockApi, MockQuerier};
use cosmwasm_std::{to_json_binary, Addr, Binary, Coin, Event, Uint128, WasmMsg};
...
fn register_bvs() {
    ...
    // let msg = bvs_directory::msg::ExecuteMsg::RegisterBVS {
    //     bvs_contract: bvs_contract.clone(),
    // };

    let msg = WasmMsg::Execute {
        contract_addr: mock_env1.bvs_directory.contract_addr.to_string(),
        msg: to_json_binary(&bvs_directory::msg::ExecuteMsg::RegisterBVS {
            bvs_contract: bvs_contract.clone(),
        })
        .unwrap(),
        funds: vec![],
