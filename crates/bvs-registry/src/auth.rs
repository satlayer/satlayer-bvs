...
#[cfg(test)]
mod tests {
    use crate::auth::{get_delegation_manager, set_routing, DELEGATION_MANAGER};
    use crate::error::ContractError;
    use bvs_library::ownership;
    ...
    fn test_get_delegation_manager() {
        ...
        DELEGATION_MANAGER
            ...
            .unwrap();
        let addr = get_delegation_manager(&deps.storage).unwrap();
    ...
    fn test_get_delegation_manager_unauthorized() {
        ...
        let err = get_delegation_manager(&deps.storage).unwrap_err();
