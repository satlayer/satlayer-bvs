...
pub fn instantiate(
    ...
    // let's quey this vault's denom balance just as a sanity check
    bank::query_balance(&deps.as_ref(), &env)?;

    let receipt_token_instantiate = ReceiptCw20InstantiateMsg {
        name: format!("{} Staking Receipt", msg.denom),
        symbol: format!("st{}", msg.denom),
        decimals: 6, // <-- there are no standard for bank denom, 6 is a good default, consider
        // defining explicitly in the contract
...
pub fn execute(
    ...
    match msg {
        CombinedExecuteMsg::WithdrawTo(msg) => vault_execute::withdraw_to(deps, env, info, msg),
        CombinedExecuteMsg::DepositFor(msg) => vault_execute::deposit_for(deps, env, info, msg),
        ...
        _ => receipt_cw20_execute::execute_base(deps, env, info, msg).map_err(Into::into),
    }
}
...
mod vault_execute {
    ...
    pub fn withdraw_to(
        ...
        let (vault, claim_assets) = {
            ...
            let assets = vault.shares_to_assets(receipt_tokens)?;
            if assets.is_zero() {
        ...
        let transfer_msg = bank::bank_send(deps.storage, &msg.recipient, claim_assets)?;
    ...
    pub fn redeem_withdrawal_to(
        ...
        let transfer_msg = bank::bank_send(deps.storage, &msg.0, claimed_assets)?;
