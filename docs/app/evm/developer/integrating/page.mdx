...
## Prerequisites
Read through [SatLayer's EVM core contracts](/evm/contracts) and understand how [Services](/architecture/services) works.
...
```solidity filename="contract.sol" copy
function updateMetadata(address operator) external {
...
The [SLAYRouterV2](/evm/contracts/ISLAYVaultV2.sol) is a central contract
...
function slashOperator(address operator) external {
    ...
    bytes32 slashId = SLAYRouterV2(REGISTRY_CONTRACT_ADDR).requestSlashing(payload);
}
```
...
function cancelSlash(bytes32 slashId) external {
    SLAYRouterV2(REGISTRY_CONTRACT_ADDR).cancelSlashing(slashId);
}
```
...
function lockSlashing(bytes32 slashId) external {
    SLAYRouterV2(REGISTRY_CONTRACT_ADDR).lockSlashing(slashId);
}
```
...
function finalizeSlashing(bytes32 slashId) external {
    SLAYRouterV2(REGISTRY_CONTRACT_ADDR).finalizeSlashing(slashId);
}
```
