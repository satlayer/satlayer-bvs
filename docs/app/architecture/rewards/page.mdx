...
provided by [Services](/architecture/services) to [Operators](/architecture/operators) and Stakers for their participation
...
Services should call the `DistributeRewards` function on the BVS Rewards contract
...
- Appropriate allowances are granted to the rewards contract
...
Earners call the `ClaimRewards` function on the [`bvs-rewards`](/cosmwasm/contracts/rewards) contract.
