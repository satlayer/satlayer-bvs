...
connWrapped = await eth.deployContract({ 
  ...
  const sDeposit = {
    wrapper: wrapper.contractAddress,
    connector: connWrapped.contractAddress,
    safety: {
  ...
  await CG.write.setStrategy([STRAT_WRAP_ID, { kind: 1, deposit: sDeposit, borrow: sBorrow }]); // kind=DepositWrap1to1
  await eth.mineBlock(1);
