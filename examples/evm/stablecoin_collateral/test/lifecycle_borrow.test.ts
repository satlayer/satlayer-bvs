...
import { abi as bvsAbi, bytecode as bvsBytecode } from "../out/StablecoinCollateralBVS.sol/StablecoinCollateralBVS.json";
...
import { abi as adapterAbi, bytecode as adapterBytecode } from "../out/StablecoinFull.t.sol/MockBorrowVenueAdapter.json";
...
const sel_unwindDepositAny    = "0x" + Buffer.from("unwindDepositAny(address,bytes32,uint256)").toString("hex").slice(0,8);
...
class SimpleAbiRegistry implements TargetAbiRegistry {
  ...
  add(target: `0x${string}`, abi: any[]) { this.map.set(target, abi); }
  getAbi(target: `0x${string}`) { return this.map.get(target) ?? null; }
}
...
cg = await eth.deployContract({ 
    ...
    salt:  salt,
    constructorArgs: [owner.address, operator.address, operator.address, pl.contractAddress, oracle.contractAddress, BASE],
});
...
test("BVS-driven: claimTo then unwindDepositAny (wrap path) → fully unlockable", async () => {
  ...
  const depositHash = await step("deposit", Vault.write.deposit([parseUnits("500", 8), alice.address], { account: alice }));
...
//   );
  ...
  const unwindPrefix = encodeAbiParameters(
    ...
    [{ type: "address" }, { type: "bytes32" }, {type: "uint256"}, {type: "uint256"}, {type: "bytes"},{type: "uint256"} ],
