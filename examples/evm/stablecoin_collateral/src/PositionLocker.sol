...
interface IConversionGatewayMulti {
  ...
  function unwindWrapAny(address user, bytes32 strategy, uint256 requestedBaseOrWrapped, uint256 minOutAfterUnwrap) external;
...
contract PositionLocker is AccessControl, ReentrancyGuard, Pausable {
    ...
    struct SubPos { uint256 allocatedShares; uint256 transformedAssets; uint256 debtAssets; }
    struct Position { uint256 totalShares; uint256 transformedTotal; mapping(StrategyId => SubPos) sub; }

    mapping(address => Position) internal positions;

    // Requests
    struct RequestInfo { address owner; StrategyId strategy; uint256 shares; uint256 createdAt; }
    ...
    function syncOperator() external { _syncOperatorInternal(); }
        ...
        external onlyRole(ROLE_GOVERNANCE)
    {
        ...
        perUserCapBps = _perUser; globalCapBps = _global; epochRateBps = _epochRate; epochLength = _epochLen;
    ...
    function setStrategyEnabled(StrategyId s, bool enabled) external onlyRole(ROLE_GOVERNANCE) {
        strategyEnabled[s] = enabled; emit StrategyEnabled(s, enabled);
    }
    ...
    function setDustAndBuffer(uint256 _dust, uint16 _bufferBps) external onlyRole(ROLE_GOVERNANCE) {
        ...
        dustThreshold = _dust; bufferBps = _bufferBps;
    }

    function setPaused(bool on) external onlyRole(ROLE_PAUSER) {
        if (on) _pause(); else _unpause();
    ...
    function requestFor(address user, uint256 shares, StrategyId strategy)
        external onlyRole(ROLE_KEEPER) nonReentrant whenNotPaused returns (uint256 reqId)
    {
        require(strategyEnabled[strategy], "STRAT_DISABLED");
    ...
    function claimTo(uint256 reqId)
        external onlyRole(ROLE_KEEPER) nonReentrant whenNotPaused returns (uint256 assetsOut)
    {
        ...
        IConversionGatewayMulti(conversionGateway).onClaimWithStrategy(r.owner, assetsOut, StrategyId.unwrap(r.strategy));
    }
    ...
    function repayAndRestake(address user, uint256 assets, StrategyId strategy)
        external onlyRole(ROLE_CG) nonReentrant returns (uint256 sharesOut)
    {
        ...
        strategyTransformed[StrategyId.unwrap(strategy)] = strategyTransformed[StrategyId.unwrap(strategy)] > assets ? strategyTransformed[StrategyId.unwrap(strategy)] - assets : 0;
    ...
    function userTotals(address user) external view returns (
        uint256 totalShares, uint256 transformedTotal, uint256 epochStart_, uint256 epochOutflow_
    ) {
        Position storage P = positions[user];
