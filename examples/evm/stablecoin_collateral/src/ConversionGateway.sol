...
contract ConversionGateway is AccessControl, ReentrancyGuard {
    ...
    struct BorrowSafety {
        ...
        uint maxAprBps;             // 0 = ignore; otherwise block if APR > maxAprBps
        uint minHfBps;              // 0 = ignore; otherwise block if HF < minHfBps
        // emergency overrides
        bool   emergencyMode;
    ...
    event UnwoundBorrow(address indexed user, bytes32 indexed strategy, uint256 debtRedeemed, uint256 debtRepaid, uint256 collateralRestaked);
    ...
    function setBorrowEmergency(bytes32 strat, bool on)
        external
    ...
    function setDepositEmergency(bytes32 strat, bool on)
        external
        onlyRole(ROLE_GOVERNANCE)
    {
    ...
    function setStrategy(bytes32 id, StrategyCfg calldata s)
        external
        onlyRole(ROLE_GOVERNANCE)
    {
        if (s.kind == RouteKind.DepositIdentity) {
            ...
            require(s.deposit.connector != address(0), "IDENT_NEEDS_CONNECTOR");
            require(
                IExternalVaultConnector(s.deposit.connector).asset() == address(baseAsset),
                "CONNECTOR_NEEDS_BASE_ASSET"
            );
            ...
            require(
                s.borrow.adapter == address(0) &&
                s.borrow.debtAsset == address(0) &&
            ...
            require(IWrapper1to1(s.deposit.wrapper).base() == address(baseAsset), "WRAP_BASE_MISMATCH");
            require(
                IExternalVaultConnector(s.deposit.connector).asset()
                    == IWrapper1to1(s.deposit.wrapper).wrapped(),
                "CONNECTOR_ASSET_MISMATCH"
            );
            _validateDepositSafety(s.deposit.safety);
            ...
            require(
                s.deposit.wrapper == address(0) && s.deposit.connector == address(0),
                "DEPOSIT_CFG_NOT_EMPTY"
            );
    ...
    * ---------------------------------------------------------------- */
    function _onClaimWithStrategyDeposit(
        address user,
        uint256 baseIn,
        bytes32 strategy
    ) internal  {
    ...
    * ---------------------------------------------------------------- */
    ...
    function unwindDepositAny(
        ...
        (uint256 outWrappedOrBase, ) =
            IExternalVaultConnector(cfg.deposit.connector).redeemFor(user, toRedeem, minOut);
        require(outWrappedOrBase >= minOut, "REDEEM_SHORTFALL");
        ...
        IPL(pl).repayAndRestake(user, baseOut, strategy);
        if (
            IExternalVaultConnector(cfg.deposit.connector).assetsOf(user) == 0
        ) {
    ...
    function _onClaimWithStrategyBorrow(
        ...
        {
            ( , bytes memory adapterData, bytes memory connectorData, ) = abi.decode(params, (uint16, bytes, bytes, uint256));
    ...
    function violatesCeilings(
        BorrowSafety memory S,
        IBorrowVenueAdapter adapter,
        address debtAsset
    ) public view returns (bool ok, uint8 reason) {
        if (S.emergencyMode) return (false, 1);

        (bool hasApr, uint aprBps, bool haveHf, uint hfBps) = adapter.getRiskSignals(debtAsset);
...
// assumes same imports, state, events
    ...
    function _repayVenueDebt(
        IBorrowVenueAdapter adapter_,
        IERC20 debt,
        uint256 debtIn,
        bytes calldata adapterData
    ) internal returns (uint256 repaidDebt, uint256 beforeDebt)
    {
