...
abstract contract BorrowAdapterBase is IBorrowVenueAdapter, AccessControl, Pausable {
    ...
    function setPaused(bool on) external onlyRole(ROLE_GOV) { on ? _pause() : _unpause(); }
    ...
    function supplyCollateral(address collateral, uint256 amount, bytes calldata data)
        external onlyRole(ROLE_CG) whenNotPaused override
    {
    ...
    function _getRiskSignals(address debtAsset) internal view virtual returns (bool hasApr, uint aprBps, bool haveHf, uint hfBps);
