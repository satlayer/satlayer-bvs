...
export async function offChainRewardTrigger(
  underlyingAssetContractAddress: `0x${string}`,
  bvsContractAddress: `0x${string}`,
  earners: Earner[],
  ...
  const distributionFileData = await dumpRewardDistribution(
    underlyingAssetContractAddress,
    bvsContractAddress,
    earners,
  );
  const distDir = `/dist/eth-mainnet/${bvsContractAddress}/${underlyingAssetContractAddress}/distribution.json`;
...
async function dumpRewardDistribution(
  ...
  for (let earner of earners) {
    let stakerRewardsAmount = calculateReward(earner.staked as bigint); // convert to uStake
    stakerRewardsMap.set(earner.account.address, stakerRewardsAmount);
    cumulativeReward += stakerRewardsAmount;
    totalStaked += earner.staked;
  }

  // convert to distribution.json file format
  let newDistributionFileData: DistributionRewards = {
    token: underlyingAssetContractAddress,
  ...
  const distDir = path.resolve(
    process.cwd(),
    `dist/eth-mainnet/${bvsContractAddress}/${underlyingAssetContractAddress}`,
  );
