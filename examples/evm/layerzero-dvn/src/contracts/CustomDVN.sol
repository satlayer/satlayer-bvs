...
import {ReadLib1002Mock as ReadLib1002} from
    "@layerzerolabs/test-devtools-evm-foundry/contracts/mocks/ReadLib1002Mock.sol";
...
contract CustomDVN is Worker, IDVN {
    ...
    {
        totalFee = 0;
        emit PacketAssigned(keccak256(abi.encodePacked(_param.packetHeader, _param.payloadHash)), totalFee);
    }
    ...
    function assignJob(address _sender, bytes calldata, /*_packetHeader*/ bytes calldata _cmd, bytes calldata _options)
        external
        payable
        onlyRole(MESSAGE_LIB_ROLE)
        onlyAcl(_sender)
    ...
    // @param _cmd - the command to be executed to obtain the payload
    // @param _options - options
    function getFee(address _sender, bytes calldata, /*_packetHeader*/ bytes calldata _cmd, bytes calldata _options)
        external
        view
        returns (uint256 fee)
    {
