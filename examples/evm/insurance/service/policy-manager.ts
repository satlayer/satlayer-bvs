...
import { mockerc20 } from "@satlayer/testcontainers";
...
export class PolicyManager {
  ...
  constructor(
    ...
    private readonly service: `0x${string}`,
    private readonly operator: `0x${string}`,
    private readonly underlyingAsset: `0x${string}`, // address of the underlying asset
    private readonly rewardsToken: `0x${string}`, // address of the rewards token
  ) {}
   ...
   */
  public async buyPolicy(coverage: number, insuree: `0x${string}`): Promise<PolicyDetails> {
    ...
    let totalVaultBalanceWithCapacity = totalVaultBalance * BigInt(this.CAPACITY_FACTOR * 100);
    if ((this.totalInsured + coverage) * 100 > totalVaultBalanceWithCapacity) {
  ...
  public async claimPolicy(policyId: number): Promise<ClaimId> {
    ...
    let slashingRequestId = await this.api.executeRequestSlashing({
      payoutAmount,
  ...
  public async processClaim(claimId: ClaimId): Promise<PolicyPayout> {
    ...
    let payoutAmount = policy.coverage;
    let res = await this.api.Client.writeContract({
      abi: mockerc20.abi,
      address: this.underlyingAsset,
      functionName: "transfer",
      args: [policy.insuree, payoutAmount],
      account: this.service,
      chain: this.api.Client.chain,
    });
   ...
   *
   * stakerEarners are passed in as it requires indexers to fetch the stakers
   */
  public async calculateRewards(stakerEarners: `0x${string}`[]): Promise<RewardsCalculationResult> {
    let now = Date.now();
    ...
    for (let earner of stakerEarners) {
      ...
      let stakerShare = await this.api.queryStakedAmount({ address: earner });
      stakerBalanceMap.set(earner, stakerShare);
    ...
    const distDir = path.resolve(process.cwd(), `dist/eth-mainnet/${this.api.Service}/${this.rewardsToken}`);
  ...
  public async submitRewards({ amountToDistribute }: { amountToDistribute: number }): Promise<string> {
    ...
    let merkleTreeRoot = await this.createMerkleTree(
      `dist/eth-mainnet/${this.api.Service}/${this.rewardsToken}/distribution.json`,
    );
    let merkleTreeRootHex = `0x${merkleTreeRoot}`;

    // increase allowance for the rewards contract to transfer the rewards amount
    await this.api.Client.writeContract({
      abi: mockerc20.abi,
      address: this.rewardsToken,
      functionName: "approve",
      args: [this.api.RewardsContract.address, amountToDistribute],
      account: this.service,
      chain: this.api.Client.chain,
    });
    ...
    await this.api.executeDistributeRewards({
      ...
      merkleRoot: merkleTreeRootHex as `0x${string}`,
    });

    return merkleTreeRootHex;
  }
   ...
   */
  private async createMerkleTree(inputFile: string): Promise<string> {
