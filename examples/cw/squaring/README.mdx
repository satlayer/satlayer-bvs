...
[service](../../../getting-started/services). It introduces a simple squaring function (`n^2`) to illustrate the
...
The example below is for our CosmWasm contracts, but the principles apply to our EVM contracts as well.
...
Clone and copy contents of `./examples/squaring` [from GitHub](https://github.com/satlayer/satlayer-bvs/tree/main/examples/cw/squaring) to your local machine.
Install with `pnpm install` and `pnpm run test` to set up the environment.

```sh filename="run.sh"
cd ./examples/squaring/contract/
pnpm run install
pnpm run test
```
...
The general request-response lifecycle is as follows:

```mermaid
sequenceDiagram
    participant S as Bitcoin Validated Service (BVS)
    participant V as Verifier
    participant C as Smart Contract (On-chain)
    participant O as Operator (Off-chain)

    S->>C: Submit computation request (e.g. square n)
    Note right of C: Request stored on-chain

    O->>C: Polls for new requests
    C-->>O: Returns pending requests

    O->>O: Performs off-chain computation (n^2)
    O->>C: Submit result to contract
    V-->>C: Verify result

    alt Correct Result
        V-->>C: Accept result
    else Incorrect Result
        V->>C: Trigger slashing
        C-->>O: Slash operator stake
    end
```
...
5. Verifiers (generally, anyone can be a verifier) verify the results and can initiate slashing on operators who submit incorrect answers

Should slashing occur, this will follow the existing slashing mechanism in the SatLayer ecosystem, where there are
guardrails in place to prevent abuse and ensure slashing is only triggered in cases of clear misbehavior, wherever defined.
